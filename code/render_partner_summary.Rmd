...
screened <- readRDS(here(paste("data/wrangled/wrangled_raw", zyear, sep = "_")))

# input the colony code and species you want to generate an observer summary sheet for

# zcode = 31.1

observers <- read.csv(here(paste("data/HEP_tracking_copy/observers.csv", sep = ""))) %>% 
...
seas_summary_observers <- screened$observers %>% 
  ...
  distinct(code, colony, observers, total.days, total.surveys, total.hours)
...
if(any(spp.nesting %in% c("GREG", "GBHE")) & nrow(brood_sizes) > 0 & any(!is.na(brood_sizes_wide$species))) {
