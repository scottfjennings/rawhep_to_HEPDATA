...
hep_total_nests <- readRDS(here("data/test_nest_abund")) %>% filter(code == zcode, species == zspp) %>% ungroup()

mean_historic_abund <- mean(hep_total_nests$peakactvnsts)
...
flextable(brood_sizes_wide) %>% 
  border_remove() %>% 
  add_header_row(values = c("", "Number of stage 4 nests with brood size:"), colwidths = c(1, 5)) %>% 
  align(j = c(2:6), align = "center", part = "all") %>% 
  autofit() %>% 
    width(j = 2:6, width = 0.5) %>% 
  border(border.top = fp_border(), part = "header") %>% 
  border(border.bottom = fp_border(), part = "header", i = 1, j = 2:6) %>% 
...
disturbance <- wrangled_s123$disturbance %>% 
  ...
  mutate(year = year(as.Date(date)),
         source = disturbance_code_to_text(source),
         result = disturbance_response_to_text(result)) %>% 
  select(date, source, result, description = details)
...
predators_rewide <- predators_longer %>% 
  ...
  mutate(year = zyear,
         present = ifelse(present == 1, "yes", "no"),
         nesting = ifelse(nesting == 1, "yes", "no")) %>% 
  select(-year, -code)
...
<br>
Historic `r spp_name` nest abundance. Red point is this year's nest abundance.  
...
hep_total_nests %>%  
  ...
  #stat_smooth(aes(x = year, y = peakactvnsts), color = "black", se = FALSE, method = "lm") +
