...
historic_nest_abund4plot %>%  
  ...
  scale_x_continuous(labels = seq((floor(min(historic_nest_abund$year, na.rm = TRUE)/5)*5), (ceiling(zyear/5)*5), by = 5), 
                     breaks = seq((floor(min(historic_nest_abund$year, na.rm = TRUE)/5)*5), (ceiling(zyear/5)*5), by = 5)) +
...
if(nspp > 0) { # if there were >= 1 nest observed:
 ...
 brood_sizes <- wrangled_s123_21$brood.sizes %>% 
  filter(code == zcode, species %in% core.spp.nesting) %>% names()
