...
nspp <- distinct(just.nesters, species) %>% nrow()
...
curr_year_peakactvnsts <- wrangled_s123$nests %>% 
  filter(code == zcode, species %in% distinct(just.nesters, species)$species, peak.active == TRUE) %>% 
...
peakactive_text <- curr_year_peakactvnsts %>% 
  mutate(row.num = row_number(),
         ...
         peakactive.text = ifelse(row.num == 1,
                                  paste("At ", site_name$site.name, " the peak number of active nests was", peakactvnsts, " ", translate_bird_names(species, "alpha.code", "common.name"), " nests, observed on ", md.text, sep = ""),
                                  ifelse(row.num == max(row.num),
                                  paste(", and ", peakactvnsts, " ", translate_bird_names(species, "alpha.code", "common.name"), " nests, observed on ", md.text, sep = ""),
                                  paste(",  ", peakactvnsts, " ", translate_bird_names(species, "alpha.code", "common.name"), " nests, observed on ", md.text, sep = "")))) %>% 
...
```{r fig.width=8, fig.height=5.3 * nspp}
...
historic_nest_abund4plot <- expand.grid(species = distinct(historic_nest_abund, species)$species,
                  year = seq(min(historic_nest_abund$year), max(historic_nest_abund$year))) %>% 
  full_join(historic_nest_abund) %>% 
  group_by(species) %>% 
  mutate(lab.na = max(peakactvnsts, na.rm = TRUE)/10) %>% 
  ungroup()
...
nest_stages_df <- wrangled_s123$stages %>% 
  filter(code == zcode, species %in% distinct(just.nesters, species)$species) %>% 
...
zyear_phenology <- wrangled_s123$stages %>% 
  ...
  filter(code == zcode, species %in% distinct(just.nesters, species)$species, phen.rop == TRUE)  %>% 
...
zyear_productivity <- wrangled_s123$brood.sizes %>% 
  filter(code == zcode, species %in% distinct(just.nesters, species)$species, brd.size.date == TRUE) %>%
  mutate(brd = paste("brd", brd, sep = ""),
         year = zyear) %>% 
...
historic_productivity4plot <- expand.grid(species = distinct(historic_productivity, species)$species,
