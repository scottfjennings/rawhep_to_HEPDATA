...
phen_stages <- wrangled_s123$stages %>% 
...
We estimate the intra-seasonal timing of nesting for Great Blue Heron and Great Egret. The proportion of nests that have reached the unguarded stage (Stages 4 and 5) by the May (Great Blue Heron) or June (Great Egret) Regional Nest Observation Period is a useful metric for us to gauge the intra-seasonal timing of nesting. A larger than average proportion of unguarded nests suggests that the birds began nesting earlier that year than normal. `r phenology_text$phenology.text`   
...
if(nspp > 0) { # if there were >= 1 nest observed:
 ...
 brood_sizes <- wrangled_s123$brood.sizes %>% 
  filter(code == zcode, species %in% core.spp.nesting) %>% 
   ...
   ungroup() %>% 
   filter(tot.nests > 0)
    if(nrow(brood_sizes) > 0) { # and if there is brood size data:
      # pivot wider for display in summary
brood_sizes_wide <- brood_sizes %>% 
  arrange(species, date, brd) %>% 
...
zyear_productivity <- wrangled_s123$brood.sizes %>% 
  ...
  mutate(brd = paste("brd", brd, sep = "")) %>% 
  pivot_wider(id_cols = c(code, species), names_from = brd, values_from = num.nests) %>% 
...
flextable(brood_sizes_wide) %>% 
  ...
  border(i = nbrood_rows$nbrood.rows, border.bottom = fp_border(), part = "body") %>% 
    set_caption(caption = "Number of Stage 4 nests of each brood size (1, 2, 3, 4, or five chicks) observed on each visit date (only dates wtih Stage 4 chicks present are shown):")
}
}
```
