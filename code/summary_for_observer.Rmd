...
spp.nesting <- distinct(just.nesters, species)$species
nspp <- length(spp.nesting)
core.spp.nesting <- (filter(just.nesters, species %in% c("GREG", "GBHE")) %>% distinct(species))$species
...
```{r fig.width=8, fig.height=ifelse(nspp > 1, 10.6, 5.3)}
historic_nest_abund <- readRDS(here("data/hep_annual_nest_abundance")) %>% 
  filter(code == zcode, species %in% spp.nesting, peakactvnsts >= 0) %>% 
  ungroup() %>% 
...
historic_nest_abund4plot <- expand.grid(species = distinct(historic_nest_abund, species)$species,
  ...
  group_by(species) %>% 
  mutate(lab.na = max(peakactvnsts, na.rm = TRUE)/15) %>% 
  ungroup()
...
```{r fig.width=8, fig.height=5.3 * nspp.core}
...
first_stage5_date <- wrangled_s123$stages %>% 
  filter(code == zcode, species %in% spp.nesting) %>%
