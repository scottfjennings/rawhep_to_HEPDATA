...
historic_nest_abund4plot %>%  
  ...
  labs(y = "Total number of nests",
       x = "",
       title = "Peak number of active nests (X = no data; * indicates value from incomplete colony count)",
       fill = "") +
  facet_wrap(~species, ncol = ceiling(distinct(historic_nest_abund4plot, species) %>% nrow()/2), scales = "free") + 
  geom_text(aes(year, peakactvnsts * 1.1, label = "*"), data = filter(historic_nest_abund4plot, complete.count == "no"))
